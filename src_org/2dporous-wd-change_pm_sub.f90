...
! FLOW VISUALIZATION
  IF (MOD(ISTEP,IPROG)==0) THEN
    ...
    ICOUNT=ICOUNT+1
    call cal_VisIns(nx, ny, nz, U, V, HU, HV, flop)
    call end_watch(18, nWatch, stop_watch)

    call begin_watch(19, nWatch, stop_watch)
    call wrt_VisIns(nx, ny, nz, istep, time, A, HU, HV, HW, flop)
...
call cpy_VisAvr(nx, ny, nz, UAVE, VAVE, HUAVE, HVAVE, flop)
call end_watch(22, nWatch, stop_watch)


call begin_watch(23, nWatch, stop_watch)
call wrt_VisAvr(nx, ny, nz, time, nstep, A, HUAVE, HVAVE, HWAVE, flop)
...
! *****************************************************
  subroutine cal_VisIns(nx, ny, nz, U, V, HU, HV, flop)

  implicit none
  real,             dimension(NX,NY,NZ)      :: HU, HV
  double precision, dimension(NX,NY)         :: U, V
...
! *********************************************************************
  subroutine cpy_VisAvr(nx, ny, nz, UAVE, VAVE, HUAVE, HVAVE, flop)

  implicit none
  real,             dimension(NX,NY,NZ)      :: HUAVE, HVAVE
  double precision, dimension(NX,NY)         :: UAVE, VAVE
  integer          :: i, j, k, nx, ny, nz
  double precision :: flop
...
!   HWAVE(I,J,K)=0.0       initialize at init_Arrays()
  ...
  return
  end subroutine cpy_VisAvr


  ! *********************************************************************
  subroutine wrt_VisAvr(nx, ny, nz, time, nstep, A, HUAVE, HVAVE, HWAVE, flop)
